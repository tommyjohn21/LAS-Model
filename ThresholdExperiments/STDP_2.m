...
%% Take only those matrices in PlasticityExperiment naive.mlx that haven't already had their Threshold computed 
% (i.e. not a member of key)

todo = setdiff(1:numel(PE.S),key);
%% Use local screen title to choose starting point
% Screens should be named 'STDP_1' and 'STDP_2'

ScreenName = evalc('!echo $STY'); % Get local screen title/name
start = str2num(ScreenName(strfind(ScreenName,'_')+1:end-1));
%% Cycle through ThresholdExperiments for weighting matrices given in key

for i = start:2:numel(todo)

    % Choose particular weighting matrix from todo
    n = todo(i);
    fprintf(['Loop %i, dW matrix %i\n'],i,todo(i)) % For logging purposes on server
