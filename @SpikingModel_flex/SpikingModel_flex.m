classdef SpikingModel_flex < SpikingNetwork
    ...
    methods % Simulation functions
        function IndividualModelUpdate(O,dt) 
            ...
            g_sum = O.param.g_L + ...
                    ...
                    (O.Input.I + O.Input.I_global) ./ (O.param.f_max.*dt) + ...
            ...
            V_inf = (O.param.g_L .* O.param.E_L  + ...
                     ...
                     (O.Input.I + O.Input.I_global) ./ (O.param.f_max.*dt) .* E_Cl + ...  
            ...
            Faraday = 96500;               
            Cl_in_inf = (O.param.tau_Cl./O.param.Vd_Cl./Faraday.*((O.Input.I+O.Input.I_global)./(O.param.f_max.*dt)).*(Veff-E_Cl) + O.param.Cl_in_eq);
...
%             Spike = logical(binornd(1,O.param.f(O.V - O.phi)));
            ...
            O.Input.I_global = O.Input.I_global.*exp(-dt./O.param.tau_syn.I_global);
    ...
    methods (Static)
        function O = SpikingModel_flex(template)
            % Model = SpikingModel(template)
            %
