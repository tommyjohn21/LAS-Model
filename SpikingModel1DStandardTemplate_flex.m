%% This is a template of spiking model
% 
...
% Divide divide grain
n = round([2000./grain,1]); % Number of neurons 
...
T_refractory = gen_param(5/time_grain,0,n,1);% ms, the duration of AP + absolutely refractory period 
...
fs = gen_param(0.002*time_grain,0,n,1); % kHz, the slope
beta = gen_param(2.5,0,n,1); % mV, high beta = low threshold noise
f = @(u) f0+fs.*exp(u./beta); % unit: kHz

dt_AP = 0.5/time_grain; % length of action potential, ms
...
tau_syn.E = gen_param(15/time_grain,0,n,0); % Excitatory synaptic time constant 
tau_syn.I = gen_param(15/time_grain,0,n,0); % Inhibitory synaptic time constant
...
tau_phi = gen_param(100/time_grain,0,n,1); % ms
...
tau_Cl = gen_param(5000/time_grain,0,n,1); % Chloride clearance time constant
Cl_in = Cl_in_eq;

% ------------- Equation 4 ---- slow afterhyperpolarization ---------------
tau_K = gen_param(5000/time_grain,0,[n,1],1); % Can also be function
